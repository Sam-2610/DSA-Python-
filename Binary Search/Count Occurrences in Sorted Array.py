...
def first_and_last(arr, n, x):
    ...
    ub = upper_bound(arr, x, n)
    return (lb, ub - 1)  # (first occurrence, last occurrence)


def count(arr, n, x):
    ans = first_and_last(arr, n, x)  # get (first, last) occurrence indices

    if ans[0] == -1:
        # x not found in arr at all
        return 0
    else:
        # count of occurrences = last index - first index + 1
